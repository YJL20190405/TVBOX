 ...
 * @returns {Promise<Array>} 弹幕列表，包含文本和属性		
 */		
async function getBilibiliDanmaku(bvid) {		
    try {		
        // 获取视频的CID
        const cidUrl = `https://api.bilibili.com/x/player/pagelist?bvid=${bvid}&jsonp=jsonp`;		
        const cidResponse = await axios.get(cidUrl);		
        ...
        // 解析XML获取弹幕内容
        const result = await parseStringPromise(danmakuResponse.data);		
        		
        const danmakuList = [];		
        if (result.i && result.i.d) {		
            for (const d of result.i.d) {		
                danmakuList.push({		
                    text: d._,		
                    attributes: d.$.p.split(',')		
                });		
            }		
        }		
...
module.exports = getBilibiliDanmaku;
