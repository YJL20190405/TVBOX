...
// 获取视频弹幕数据（需替换cid为实际视频ID）
fetch('https://api.bilibili.com/x/v1/dm/list.so?oid=1329940574')
  ...
  .then(data => {
    ...
    // 弹幕渲染逻辑
    Array.from(danmus).forEach(danmu => {
      const text = danmu.textContent;
      const p = danmu.getAttribute('p').split(',');
      const time = parseFloat(p[0]);
      const type = parseInt(p[1]);
      const fontSize = parseInt(p[2]);
      const color = parseInt(p[3]);
      
      // 创建弹幕元素并添加到播放器
      createDanmuElement(text, time, type, fontSize, color);
    });
  });

function createDanmuElement(text, time, type, fontSize, color) {
  // 具体渲染实现
}